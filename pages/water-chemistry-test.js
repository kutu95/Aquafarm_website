...
export default function WaterChemistryTest() {
  ...
  // Simplified state - only temperature needed for toxicity calculator
  ...
  const handleImageSelect = async (file, capturedDataUrl = null) => {
    ...
    if (capturedDataUrl) {
      ...
      if (extractedDate) {
        // Date extraction not needed for public version
      }
...
• Check your camera settings for image quality`);
  ...
  // Save functionality removed for public version

  const handleAnalysisComplete = (analysisResults) => {
    setResults(analysisResults);
    setIsAnalyzing(false);
    // No save functionality in public version
  };
    ...
    <Layout>
      <div className="min-h-screen bg-gray-50 py-8">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                  ...
                  >
                    <div className="flex flex-col items-center">
                      <svg className="h-8 w-8 text-blue-500 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 16a4 4 0 0 1 -0.88 -7.903A5 5 0 1 1 15.9 6L16 6a5 5 0 0 1 1 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
                      </svg>
                    ...
                    <div className="relative inline-block">
                      {showCropper ? (
                        <div className="relative">
                          ...
                          {/* Date display removed for public version */}
          ...
          {/* Temperature Input for Toxicity Calculator */}
          {results && results.parameters && results.parameters.ammonia?.value !== undefined && results.parameters.pH?.value !== undefined && (
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mt-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">🌡️ Water Temperature (for Ammonia Toxicity Calculator)</h3>
              <div className="max-w-md">
                ...
                <p className="text-xs text-gray-500 mt-2">
                  Enter the water temperature to calculate ammonia toxicity levels
                </p>
              </div>
            </div>
          )}

          {/* AI Status & Configuration - Admin Only */}
          {isAdmin && (
